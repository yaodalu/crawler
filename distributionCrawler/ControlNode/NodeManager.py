...
#Queue.Queue是进程内非阻塞队列，各自私有
#multiprocessing.Queue是跨进程通信队列，各子进程共有
...
class NodeManager(object):
        ...
        while True:
            while(url_manager.has_new_url()):
                ...
                if(url_manager.old_url_size()>100):
                    ...
                    #关闭管理节点，同时存储set状态????
                    url_manager.save_progress(r'C:\Users\1\Desktop\python_code\distributionCrawler\ControlNode\new_urls.txt',url_manager.new_urls)#??
                    url_manager.save_progress(r'C:\Users\1\Desktop\python_code\distributionCrawler\ControlNode\old_urls.txt',url_manager.old_urls)
                    return
    ...
    def result_solve_proc(self,result_q,conn_q,store_q):
        '''
        :数据处理进程
        :param:result_q 从result_q中get数据和url
        :param:conn_q 向conn_q中发送数据
        :param:store_q 向store_q中发送url
        '''
        while True:
            try:
                if not result_q.empty():
                    content = result_q.get(True)
                    if content['new_urls'] == 'end': #url_q发送的？？？？
                        #结束信号
